...
```txt
LAB7_314553032_task1_a2c_pendulum_trainbest_gamma095.pt   # 單一 training episode best_return，診斷用
LAB7_314553032_task1_a2c_pendulum_evalbest_gamma095.pt    # seeds 0-19 mean reward 最佳，評分優先用
```
...
5. 優先使用 `LAB7_314553032_task1_a2c_pendulum_evalbest_gamma095.pt` 做最終 evaluation。
...
若需要保留 one-step TD target 做 ablation，可使用 `--n-step 1` 搭配 `--update-batch-size`。這時每筆 transition 的 target 仍是 `r + gamma V(s')`，但 actor/critic 會累積多筆 one-step transition 後再更新，以降低單步 policy-gradient variance。建議先試 `--update-batch-size 32`，並加 `--max-grad-norm 0.5` 避免 batch update 梯度過大。
...
- [ ] 已產生 `LAB7_314553032_task1_a2c_pendulum_trainbest_gamma095.pt`。
- [ ] 已產生 `LAB7_314553032_task1_a2c_pendulum_evalbest_gamma095.pt`。
